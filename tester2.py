        ...
        if x_elem is not None and y_elem is not None:
            ...
            if x_text and y_text:
                try:
                    ...
                    # TM 좌표를 위도, 경도로 변환
                    lon, lat = transform(tm_proj, wgs84_proj, x, y)
                    park_names.append(bplcnm)
                    park_coords.append((lat, lon))
                except ValueError:
                    print(f"잘못된 좌표 값: {bplcnm}")
            else:
                print(f"좌표 값 없음: {bplcnm}")
...
# 종합유원지 마커 추가 (파란색)
for name, coord in zip(park_names, park_coords):
    folium.Marker(coord, popup=name, icon=folium.Icon(color="blue")).add_to(m)
